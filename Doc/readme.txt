...
2020.05.28
 + 添加printenv指令中 提示 是否在供暖期 添加 ：供暖期内、试运营前一个月、供暖期延期后一个月、非供暖期
 + 添加在试运营前一个月期间会每6个小时清除一次阀门的累计开阀时间等累计计量数据。
 + 添加通断控制器协议 协议类型62（清除上一供暖季计量数据）
 * 修改宏定义中关于 Meter_DDF2_ENABLE 关掉次设备之后 导致 ddf2ctrl 命令中代码不能编译问题 和 定义的局部变量没有使用问题。
 * 修改全部设备设定温度时，“全局设备温度设置” 改为 “系统统一设定”。
 * 修改通断控制器协议中  协议类型59中  温度设定的类型 由 0xFF 更正为 0x00.
 * 修改日志打印输出改为 9600 N 8 1·
 * 修改当一上电等待5秒钟则立即抄收设备
 * 修改提示日志中 发送服务器 的设备类型 热量表、通断控制器等。
...
Task07_F    //数据打包服务
Task08_F    //设备轮询抄收
Task09_F    //重复抄收服务  （重新抄收没有抄收到的设备）
Task10_F    //通信通道1 设备广播对时触发控制
Task12_F    //用户数据定时保存
Task13_F    //设备数据间隔上报任务  自动上报任务（采集器-->服务器  采集器返回抄收到的数据打包为丰源采集器协议上报到服务器）
