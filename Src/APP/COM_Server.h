...
/*
	耐威科楼栋单元调节阀modbus协议 主机侧通信控制模块
*/
#ifdef  Valve_NWK_ENABLE	

#define UART_TO_NWKBufferSize_S		    256							//流处理缓冲区
#define UART_TO_NWKPackSize_S		    (UART_TO_NWKBufferSize_S/4)
#define UART_TO_NWK_TimeOut_S		    50


COM_SERVER_EXT UART_RBC_Stru *UART_TO_NWK_CCB_S;					        //声明控制块指针
COM_SERVER_EXT INT8U UART_TO_NWK_Buffer_S[UART_TO_NWKBufferSize_S];	    //串行接收缓冲区
COM_SERVER_EXT INT8U UART_TO_NWK_InputPack_S[UART_TO_NWKPackSize_S];	    //入口堆栈缓冲器
COM_SERVER_EXT INT8U UART_TO_NWK_OutputPack_S[UART_TO_NWKPackSize_S];	    //出口堆栈缓冲器
COM_SERVER_EXT INT8U UART_TO_NWK_CCB_Init_S( void);
#endif
