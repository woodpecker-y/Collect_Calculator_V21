...
void devicedata(char *pcWriteBuffer, int xWriteBufferLen,int argc, char **argv)
{
	...
	else
	{
		...
		switch(DevType)	
		{
            ...
            case Meter_HT3X:
            {
                cli_printf("Meter_HT3X   海通小口径热量表 -HT3X188 \r\n");
                cli_printf("进水温度:    %d\r\n",SysDevData[devNum].Device10.Input_Temp);
                cli_printf("回水温度:    %d\r\n",SysDevData[devNum].Device10.Output_Temp);
                
                cli_printf("瞬时流量:    %lu\r\n",SysDevData[devNum].Device10.Instant_Current);
                cli_printf("    单位:    %02X\r\n",SysDevData[devNum].Device10.Instant_Current_Unit);
                
                cli_printf("瞬时热量:    %lu\r\n",SysDevData[devNum].Device10.Instant_Energy);
                cli_printf("    单位:    %02X\r\n",SysDevData[devNum].Device10.Instant_Energy_Unit);
                
                cli_printf("累计流量:    %lu\r\n",SysDevData[devNum].Device10.Total_Current);
                cli_printf("    单位:    %02X\r\n",SysDevData[devNum].Device10.Instant_Current_Unit);
                
                cli_printf("累计冷量:    %lu\r\n",SysDevData[devNum].Device10.Total_Code);
                cli_printf("    单位:    %02X\r\n",SysDevData[devNum].Device10.Total_Code_Unit);
                
                cli_printf("累计热量:    %lu\r\n",SysDevData[devNum].Device10.Total_Energy);
                cli_printf("    单位:    %02X\r\n",SysDevData[devNum].Device10.Total_Energy_Unit);
                
                cli_printf("工作时间:    %lu\r\n",SysDevData[devNum].Device10.Work_Time);
                
                cli_printf("热表状态1:   %02X\r\n",SysDevData[devNum].Device10.STATE1);
                cli_printf("热表状态2:   %02X\r\n",SysDevData[devNum].Device10.STATE2);

                cli_printf("通信总数:    %lu\r\n",SysDevStatus[devNum].Device10.ComTolNum);
                cli_printf("通讯成功:    %lu\r\n",SysDevStatus[devNum].Device10.ComSucNum);
                cli_printf("通讯失败:    %lu\r\n",SysDevStatus[devNum].Device10.ComFauNum);  
            }break;
#endif
	...
	{	
		...
		if(DeviceT !=DEFUNULL)
		{
			...
			switch(DeviceT)
			{
                ...
                case Meter_HT3X:
                {
                    COM_T =SysDevStatus[i].Device10.ComTolNum;
                    COM_S =SysDevStatus[i].Device10.ComSucNum;
                    COMBack_CE =SysDevStatus[i].Device10.ComFauNum;
                }break;
#endif
...
void devchange(char *pcWriteBuffer, int xWriteBufferLen,int argc, char **argv)
{
	...
	if(Getdevice_Num(DevID1,&DevNum) ==HAL_OK)
	{
		...
		SysDeviceList.Device[DevNum].Type =(SysDEV_Type)DevType;
...
void ClirateSet(char *pcWriteBuffer, int xWriteBufferLen,int argc, char **argv)
{
    ...
    if(argc != 2)       //判断输入参数的合法性
    {
        cli_printf("clirate [xxxxxxxx]\r\n\r\n");
        return;
    }
    rate = atoi(argv[1]);


    if((rate <9600)||(rate >921600))
    {

        cli_printf("clirate Set the value error  1200<= x <= 921600\r\n\r\n");
        return;
    }
