...
*************************************************************************************************
*/ 
#define _HT3X188PRO_C_
#include "HT3X188Pro.h"
 /* User Include Files */
	...
	switch (Proto_RX)
	{
		case 0X81:
		{
			if(HT2X188_Pack_0X129_S(Ctrl_Point) ==0x00)			//接收协议处理
			{
				...
				if(ClientCH1Ctrler.Device.Type ==Meter_HT2X)	//大口径楼栋热量表
				{
					...
					if( SysDeviceList.Device[DevNum].ID ==MeterID)	//设备查找成功
					{
						...
						/* 后台实时数据转发  自动抄收 */
							...
							if(( SysPara.SendMode ==DevSendMode_Auto)&&( ClientCH1Ctrler.UaComFlg ==0))//数据上报类型为自动跟踪 并且外部触发无效
							{
    							...
    							FY1000_Q_TX_Buffer.SendData.Pack_0XB0_D2.Dev_Type =Meter_HT2X;
						...
						/* 后台实时数据转发  远程抄收 */
							if( ClientCH1Ctrler.UaComFlg !=0)//外部触发数据超收 
							{
    							...
    							FY1000_Q_TX_Buffer.SendData.Pack_0X02_D2.Dev_Type =Meter_HT2X;
