...
UART_TO_HT2X188_QueueSend_Stru HT2X188_Q_RX_Buffer;		//通信发送队列 接收缓冲器
UART_TO_HT2X188_QueueSend_Stru* HT2X188_Q_RX_BuffeP;	//通信发送队列 接收缓冲器P




/*
void HT2X188_Pack_RxServer_S( UART_RBC_Stru* Ctrl_Point )
...
void HT2X188_Pack_RxServer_S( UART_RBC_Stru* Ctrl_Point )
{
	...
	HT2X188_Pack_Uni* HT2X188_RXPack =(HT2X188_Pack_Uni*)(Ctrl_Point->InputPack);
	while (Ctrl_Point->Rx_Rear !=Ctrl_Point->Rx_Front) 
	{
		...
		switch(Ctrl_Point->RecPackState)
		{
			...
			case 1:	//接收版本号/设备类型符
			{		
				if ( Bufer == HT2X188Pro_ProtocolCode)
				{
			...
			case 10:	//数据长度
			{
				...
				if(HT2X188_RXPack->DefaultPack.Lenth )	//判断数据内容是否存在
				{
				...
				DataBdry =HT2X188_RXPack->DefaultPack.Lenth;
			}break;
			...
			case 13:	//结束标志
			{
				...
				if (Bufer ==HT2X188Pro_EndCode)
				{
					HT2X188_Pack_RxAnalyze_S(Ctrl_Point,HT2X188_RXPack->DefaultPack.Lenth+HT2X188Pro_HeadSize);
				}
		...
		if (++Ctrl_Point->Rx_Rear >= UART_TO_HT2X188BufferSize_S)
		{
...
INT8U HT2X188_Pack_RxAnalyze_S(UART_RBC_Stru* Ctrl_Point,INT8U DataSize)
{
	...
	HT2X188_Pack_Uni* Packin =(HT2X188_Pack_Uni*)Ctrl_Point->InputPack;	//指针变换
	...
	else
	{
		...
		CheckFlg =HT2X188_Pack_Rx_S(Ctrl_Point,ProtoNO);
	}
	return CheckFlg;
}
...
INT8U HT2X188_Pack_Rx_S(UART_RBC_Stru* Ctrl_Point,INT8U Protocol)
{
	...
	INT8U ErrorFlg =0;
	HT2X188_Pack_Uni* PackData =(HT2X188_Pack_Uni*)(Ctrl_Point->InputPack);		   	//输入指针交接
	...
	switch (Proto_RX)
	{
		case 0X81:
		{
			if(HT2X188_Pack_0X129_S(Ctrl_Point) ==0x00)			//接收协议处理
			{

				if((ClientCH1Ctrler.Device.Type ==Meter_HT2X)||( ClientCH1Ctrler.Device.Type ==Meter_HT3X ))	//海通表数据类型
				{
					...
					DevType = ClientCH1Ctrler.Device.Type;  //获取设备类型

					
					if( SysDeviceList.Device[DevNum].ID ==MeterID)	//设备查找成功
					{
						//累计冷量
						DataBuffer4 =PackData->Pack0X129S.CODE_Energy;
						SysDevData[DevNum].Device7.Total_Code=BcdToHex4(&DataBuffer4);//日累计热量
						SysDevData[DevNum].Device7.Total_Code_Unit=PackData->Pack0X129S.CODE_Energy_Unit;//热量单位


						//当前热量
						DataBuffer4 =PackData->Pack0X129S.Total_Energy;
						SysDevData[DevNum].Device7.Total_Energy =BcdToHex4(&DataBuffer4);//热量值
						SysDevData[DevNum].Device7.Total_Energy_Unit =PackData->Pack0X129S.Total_Energy_Unit;//热量单位

						//热功率
						DataBuffer4 =PackData->Pack0X129S.Instant_Energy;
						SysDevData[DevNum].Device7.Instant_Energy =BcdToHex4(&DataBuffer4);//热功率
						SysDevData[DevNum].Device7.Instant_Energy_Unit =PackData->Pack0X129S.Instant_Energy_Unit;//热功率单位

						//瞬时流量
						DataBuffer4 =PackData->Pack0X129S.Instant_Current;
						SysDevData[DevNum].Device7.Instant_Current =BcdToHex4(&DataBuffer4);//流速
						SysDevData[DevNum].Device7.Instant_Current_Unit =PackData->Pack0X129S.Instant_Current_Unit;//流速单位

						//累计流量采集
						DataBuffer4 =PackData->Pack0X129S.Total_Current;						
						SysDevData[DevNum].Device7.Total_Current =BcdToHex4(&DataBuffer4);//累计流量
						SysDevData[DevNum].Device7.Total_Current_Unit =PackData->Pack0X129S.Total_Current_Unit;//累计流量
						...
						SysDevData[DevNum].Device7.Input_Temp =TempH*10000+TempL;
						...
						TempH =BcdToHex(DataBuffer);
						SysDevData[DevNum].Device7.Output_Temp =TempH*10000+TempL;
						...
						SysDevData[DevNum].Device7.Work_Time =WorkTime_H*10000UL+WorkTime_L;


						//仪表运行状态监测
						//仪表状态第一个字节
						SysDevData[DevNum].Device7.STATE1 =PackData->Pack0X129S.STATE1;//仪表状态
	 
						//仪表状态第二个字节
						SysDevData[DevNum].Device7.STATE2 =PackData->Pack0X129S.STATE2;//仪表状态 

						SysDevData[DevNum].Device7.Apportion_Energy =SysDevData[DevNum].Device7.Total_Energy -SysDevData[DevNum].Device7.CycBot_Energy;  //周期热量计算
						SysDevStatus[DevNum].Device7.ComSucNum +=1;
						SysDevStatus[DevNum].Device7.ComFauNum =0;
						...
						if( SysPara.DeviceType !=Type_Valve)	//非通断时间面积法热计量系统
						{
							...
							{
								UserData[UserNum].Apportion_Energy =SysDevData[DevNum].Device7.Total_Energy; //将阀门开阀数据传递给用户
						...
						/* 后台实时数据转发  自动抄收 */
							...
							if(( SysPara.SendMode ==DevSendMode_Auto)&&( ClientCH1Ctrler.UaComFlg ==0))//数据上报类型为自动跟踪 并且外部触发无效
							{
    							...
    							FY1000_Q_TX_Buffer.SendData.Pack_0XB0_D2.Dev_Type = DevType;
						...
						/* 后台实时数据转发  远程抄收 */
							if( ClientCH1Ctrler.UaComFlg !=0)//外部触发数据超收 
							{
    							...
    							FY1000_Q_TX_Buffer.SendData.Pack_0X02_D2.Dev_Type =DevType;
    							FY1000_Q_TX_Buffer.SendData.Pack_0X02_D2.Dev_ID =MeterID;
    							FY1000_Q_TX_Buffer.SendData.Pack_0X02_D2.BackFlag =COMBack_OK;

    							FY1000_Q_TX_Buffer.SendData.Pack_0X02_D2.Input_Temp =SysDevData[DevNum].Device7.Input_Temp;
    							FY1000_Q_TX_Buffer.SendData.Pack_0X02_D2.Output_Temp =SysDevData[DevNum].Device7.Output_Temp;
    							
    							FY1000_Q_TX_Buffer.SendData.Pack_0X02_D2.Instant_Current =SysDevData[DevNum].Device7.Instant_Current;
    							FY1000_Q_TX_Buffer.SendData.Pack_0X02_D2.Instant_Current_Unit =SysDevData[DevNum].Device7.Instant_Current_Unit;
    							
    							FY1000_Q_TX_Buffer.SendData.Pack_0X02_D2.Instant_Energy =SysDevData[DevNum].Device7.Instant_Energy;
    							FY1000_Q_TX_Buffer.SendData.Pack_0X02_D2.Instant_Energy_Unit =SysDevData[DevNum].Device7.Instant_Energy_Unit;
    							
    							FY1000_Q_TX_Buffer.SendData.Pack_0X02_D2.Total_Current =SysDevData[DevNum].Device7.Total_Current;
    							FY1000_Q_TX_Buffer.SendData.Pack_0X02_D2.Total_Current_Unit =SysDevData[DevNum].Device7.Total_Current_Unit;
    							
    							FY1000_Q_TX_Buffer.SendData.Pack_0X02_D2.Total_Code =0;
    							FY1000_Q_TX_Buffer.SendData.Pack_0X02_D2.Total_Code_Unit =0;
    							
    							FY1000_Q_TX_Buffer.SendData.Pack_0X02_D2.Total_Energy =SysDevData[DevNum].Device7.Total_Energy;
    							FY1000_Q_TX_Buffer.SendData.Pack_0X02_D2.Total_Energy_Unit =SysDevData[DevNum].Device7.Total_Energy_Unit;
    							
    							FY1000_Q_TX_Buffer.SendData.Pack_0X02_D2.Work_Time =SysDevData[DevNum].Device7.Work_Time;
    							FY1000_Q_TX_Buffer.SendData.Pack_0X02_D2.STATE1 =SysDevData[DevNum].Device7.STATE1;
    							FY1000_Q_TX_Buffer.SendData.Pack_0X02_D2.STATE2 =SysDevData[DevNum].Device7.STATE2;
...
*/
INT8U HT2X188_Pack_TxServer_S(UART_RBC_Stru* Ctrl_Point)
{
	...
	/* 接收队列数据 */
	Err =xQueueReceive(UART_TO_HT2X188_Msg,&HT2X188_Q_RX_BuffeP,5);
	if( Err != pdTRUE)
	{
		return HAL_ERROR;
	}
	...
	/* 复制待发送数据并打包 */
	HT2X188_Q_RX_Buffer =*HT2X188_Q_RX_BuffeP;
	Ctrl_Point->PackCreatFlag =HT2X188_Q_RX_Buffer.PackCreatFlag;
	Ctrl_Point->PackINPort =HT2X188_Q_RX_Buffer.PackINPort;
	...
	switch(Ctrl_Point->PackINPort)
	{
		case 0X01:
		{
			HT2X188_Pack_0X01_S(Ctrl_Point);
...
INT8U HT2X188_Pack_0X129_S(UART_RBC_Stru* PORT)
{
	...
	HT2X188_Pack_Uni* PackData =(HT2X188_Pack_Uni*)(PORT->InputPack);	//指针变换
	INT8U BackVal =0X00;
...
void HT2X188_Pack_0X01_S(UART_RBC_Stru* PORT)
{
	...
	HT2X188_Pack_Uni* PackData =(HT2X188_Pack_Uni*)(PORT->OutputPack);	//指针变换
	INT8U PackSize =0;
	INT8U Type =0;
	...
	PackData->Pack0X01S.Head.SN =HT2X188_Q_RX_Buffer.ID;

    Type = HT2X188_Q_RX_Buffer.Type;

    if(Type == Meter_HT2X)
    {
        PackData->Pack0X01S.Head.FAdd1=0X00;
        PackData->Pack0X01S.Head.FAdd2=0X0000;
        
    }
    else if( Type == Meter_HT3X)
    {
        PackData->Pack0X01S.Head.FAdd1=0X11;
        PackData->Pack0X01S.Head.FAdd2=0X0011;
    }
    else
    {
	...
	PackData->Pack0X01S.Head.Lenth =sizeof(HT2X188_Pack01S_Stru)-HT2X188Pro_HeadSize-2-4;//实际有效数据区长度
	PackSize =sizeof(HT2X188_Pack01S_Stru)-2;	//校验数据长度计算
	


	PackData->Pack0X01S.Flag =0X1F90;
