...
/*END*/
    ...
    //Task01
    xTaskCreate((TaskFunction_t )Task01_F,				//任务函数  
    ...
    //Task02
    xTaskCreate((TaskFunction_t )Task02_F,				//任务函数  
    ...
    //Task03
    xTaskCreate((TaskFunction_t )Task03_F,				//任务函数  
                (const char*	)"UI_KEY",   		//任务名称
                (uint16_t	)Task03_StkSize,			//任务堆栈
                (void*		)NULL,			//任务参数
                (UBaseType_t	)Task03_Prio,		//任务优先级
                (TaskHandle_t*	)&Task03_Handle);	//任务句柄


    //Task04
    xTaskCreate((TaskFunction_t )Task04_F,				//任务函数  
                (const char*	)"UI_LCD",   	//任务名称
                (uint16_t	)Task04_StkSize,			//任务堆栈
                (void*		)NULL,			//任务参数
                (UBaseType_t	)Task04_Prio,		//任务优先级
                (TaskHandle_t*	)&Task04_Handle);	//任务句柄

    //Task05
    xTaskCreate((TaskFunction_t )Task05_F,				//任务函数  
                (const char*	)"UART_RBC_Rx",  //任务名称
                (uint16_t	)Task05_StkSize,			//任务堆栈
                (void*		)NULL,			//任务参数
                (UBaseType_t	)Task05_Prio,		//任务优先级
                (TaskHandle_t*	)&Task05_Handle);	//任务句柄


    //Task06
    xTaskCreate((TaskFunction_t )Task06_F,				//任务函数  
                (const char*	)"UART_RBC_Tx",  //任务名称
                (uint16_t	)Task06_StkSize,			//任务堆栈
                (void*		)NULL,			//任务参数
                (UBaseType_t	)Task06_Prio,		//任务优先级
                (TaskHandle_t*	)&Task06_Handle);	//任务句柄


    //Task07
    xTaskCreate((TaskFunction_t )Task07_F,				//任务函数  
                (const char*	)"UART_Packing", //任务名称
                (uint16_t	)Task07_StkSize,			//任务堆栈
                (void*		)NULL,			//任务参数
                (UBaseType_t	)Task07_Prio,		//任务优先级
                (TaskHandle_t*	)&Task07_Handle);	//任务句柄


    //Task08
    xTaskCreate((TaskFunction_t )Task08_F,				//任务函数  
                (const char*	)"ClientSend",//任务名称
                (uint16_t	)Task08_StkSize,			//任务堆栈
                (void*		)NULL,			//任务参数
                (UBaseType_t	)Task08_Prio,		//任务优先级
                (TaskHandle_t*	)&Task08_Handle);	//任务句柄        


    //Task09
    xTaskCreate((TaskFunction_t )Task09_F,				//任务函数  
                (const char*	)"ClientCtrler", //任务名称
                (uint16_t	)Task09_StkSize,			//任务堆栈
                (void*		)NULL,			//任务参数
                (UBaseType_t	)Task09_Prio,		//任务优先级
                (TaskHandle_t*	)&Task09_Handle);	//任务句柄     

              
    //Task10
    xTaskCreate((TaskFunction_t )Task10_F,				//任务函数  
                (const char*	)"Task10_N",   	//任务名称
                (uint16_t	)Task10_StkSize,			//任务堆栈
                (void*		)NULL,			//任务参数
                (UBaseType_t	)Task10_Prio,		//任务优先级
                (TaskHandle_t*	)&Task10_Handle);	//任务句柄 
...
*/
	...
	if(CLI_R_OK == cli_res)
	{
		xTaskCreate((TaskFunction_t )TaskCli,				//任务函数  
					(const char*	)"TaskCli",   	//任务名称
					(uint16_t	)TaskCli_StkSize,			//任务堆栈
					(void*		)NULL,			//任务参数
					(UBaseType_t	)TaskCli_Prio,		//任务优先级
					(TaskHandle_t*	)&TaskCli_Handle);	//任务句柄     
	}
    ...
    //Task12  
    xTaskCreate((TaskFunction_t )Task12_F,					//任务函数  
                (const char*	)"Userdata_Send",   	//任务名称
                (uint16_t	)Task12_StkSize,				//任务堆栈
                (void*		)NULL,				//任务参数
                (UBaseType_t	)Task12_Prio,			//任务优先级
                (TaskHandle_t*	)&Task12_Handle);		//任务句柄    


    //Task13  
    xTaskCreate((TaskFunction_t )Task13_F,					//任务函数  
                (const char*	)"Devdata_Send",   	//任务名称
                (uint16_t	)Task13_StkSize,				//任务堆栈
                (void*		)NULL,				//任务参数
                (UBaseType_t	)Task13_Prio,			//任务优先级
                (TaskHandle_t*	)&Task13_Handle);		//任务句柄   
...
void Task01_F(void const * argument)
{
    ...
    while(1)
    {
        ...
        if( SysOnbootFlag ==1)
        {
			...
			dbg_printf(DEBUG_INFO,"重启倒计时 %d...",RemoRebotCont-(SystemTick_ms -SysOnbootdelay)/1000);				
...
#if 0
	SysDeviceList.Device[8].Type =Meter_DDF2;
	SysDeviceList.Device[8].ID =0x05011768 ;
	SysDeviceList.Device[8].Num =8;
#endif
    while(1)
    {
		feedIWDG(Iwdg_T8);
						...
						switch(DevType)
						{
							case Meter_B:
							{
								switch(ClientCH1Ctrler.UaComFlg)
								{
									case 0:	//自动抄收  
									{
										...
										dbg_printf(DEBUG_WARNING,"设备自动抄收失败，类型:%d 编号:%08X...",DevType,ClientCH1Ctrler.Device.ID);
									}break;
									case 0XAAAA: //远程抄收
									{
										...
										dbg_printf(DEBUG_WARNING,"设备远程抄收失败 0X01，类型:%d 编号:%08X...",DevType,ClientCH1Ctrler.Device.ID);
									...
									case 0XAAAA: //远程抄收/控制
									{
										switch(ClientCH1Ctrler.Uapack)
										{
											...
											case 0X0D://阀门动作控制
											{
												...
												dbg_printf(DEBUG_WARNING,"设备远程控制失败 0X0D，类型:%d 编号:%08X...",DevType,ClientCH1Ctrler.Device.ID);
                            ...
                            case Meter_HT2X:
                            {
                                switch(ClientCH1Ctrler.UaComFlg)
                                {
                                    ...
                                    {
                                        SendBuffer_1000.SendData.Pack_0XB0_F.CtrlFlag=0XAA;
                                        SendBuffer_1000.SendData.Pack_0XB0_F.Dev_Type =Meter_HT2X;
                                        SendBuffer_1000.SendData.Pack_0XB0_F.Dev_ID =ClientCH1Ctrler.Device.ID;
                                        SendBuffer_1000.SendData.Pack_0XB0_F.BackFlag=COMBack_CE;
                                    ...
                                    {
                                        SendBuffer_1000.SendData.Pack_0X02_F.CtrlFlag=0XAA;
                                        SendBuffer_1000.SendData.Pack_0X02_F.Dev_Type =Meter_HT2X;
                                        SendBuffer_1000.SendData.Pack_0X02_F.Dev_ID =ClientCH1Ctrler.Device.ID;
                                        SendBuffer_1000.SendData.Pack_0X02_F.BackFlag=COMBack_CE;
		...
		if(UseSend_Semaphore !=NULL)	//信号量有效性判断
		{
			...
			if(Err ==pdTRUE)
			{
				for(INT16U UserNum =0;UserNum <SysPara.UserNum;UserNum +=1)
				{
					...
					{
						dbg_printf(DEBUG_INFO,"用户数据保存成功 序号:%d  ID:%08X",UserNum,SysUserList.User[UserNum].User_ID);
					}
				...
				switch(DeviceType)
				{
					...
					case Valve_UH ://户用通断控制器 回水阀用
					{
						...
						//	FY1000_Q_TX_Buffer.SendData.Pack_0XB0_D4.ComFauNum =SysDevStatus[DevNum].Device4.ComFauNum;
							...
							if(FY_1000Send_Code_QInput(&FY1000_Q_TX_Buffer,0XA4)==pdTRUE)
							{
								dbg_printf(DEBUG_DEBUG,"设备数据循环发送  编号: %d SN:%08X",DevNum,FY1000_Q_TX_Buffer.SendData.Pack_0XB0_D4.Dev_ID);
							}						
							osDelay(SysPara.SendSpace);
						}
					...
					case Meter_XY ://用户超声波热量表 XY-188
					{
						...
						//	FY1000_Q_TX_Buffer.SendData.Pack_0XB0_D2.ComFauNum =SysDevStatus[DevNum].Device6.ComFauNum;
							...
							if(FY_1000Send_Code_QInput(&FY1000_Q_TX_Buffer,0XA2)==pdTRUE)
							{
								dbg_printf(DEBUG_DEBUG,"设备数据循环发送  编号: %d SN:%08X\r\n ",DevNum,FY1000_Q_TX_Buffer.SendData.Pack_0XB0_D2.Dev_ID);
							}
							osDelay(SysPara.SendSpace);
