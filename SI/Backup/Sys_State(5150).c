...
void ClirateSet(char *pcWriteBuffer, int xWriteBufferLen,int argc, char **argv)
{
    ...
    if((rate <1200)||(rate >921600))
    {
        ...
        cmd_printf("clirate Set the value error  1200<= x <= 921600\r\n");
        return;
    }

    cmd_printf("clirate The rate value set success :  %d\r\n",rate);
    
    cmd_printf("Please reset the serial communication rate and turn it back on..\r\n");
    cmd_printf("If you want to restore the default settings[115200], please restart the device again.\r\n\r\n");
    cmd_printf("THANK YOU, GOOD LUCK\r\n\r\n");
